...
export class GuildMember extends Identifiable<IdentifiableGuildMember> {
    ...
    constructor(protected client: WilsonClient, public readonly guild: Guild, basic: Partial<IdentifiableGuildMember>) {
        super(client, basic);
    }
    ...
    patch(basic: Partial<IdentifiableGuildMember>) {
        ...
        if (basic.joined_at !== undefined) this.joined_at = new Date(basic.joined_at);
        if (basic.premium_since !== undefined) this.premium_since = basic.premium_since ? new Date(basic.premium_since) : null;
