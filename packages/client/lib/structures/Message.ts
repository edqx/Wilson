...
export class Message extends Identifiable<BasicMessage> {
    ...
    constructor(protected client: WilsonClient, message: Partial<BasicMessage>) {
    ...
    patch(basic: Partial<BasicMessage>) {
        ...
        if (basic.author !== undefined && this.guild) {
            ...
            if (basic.member !== undefined) {
                this.member = this.client.members.add(this.guild, basic.author.id, basic.member);
            }
        }
        if (basic.content !== undefined) this.content = basic.content;
        if (basic.timestamp !== undefined) this.created_at = new Date(basic.timestamp);
        if (basic.edited_timestamp !== undefined) this.edited_at = basic.edited_timestamp ? new Date(basic.edited_timestamp) : this.created_at;
        if (basic.tts !== undefined) this.tts = basic.tts;
        if (!this.mentions) this.mentions = {
            users: new MaskedCache(this.client, this.client.users, new Set),
            members: new MaskedCache(this.client, this.client.members, new Set),
            roles: new MaskedCache(this.client, this.client.roles, new Set),
            channels: new MaskedCache(this.client, this.client.channels, new Set),
            everyone: false
        };
        if (basic.mentions !== undefined) {
            ...
            for (const basic_user of basic.mentions) {
                ...
                if (basic_user.member && this.guild) {
                    this.client.members.add(this.guild, user.id, basic_user.member as BasicGuildMember);
        ...
        if (basic.reactions !== undefined) {
            ...
            for (const basic_reaction of basic.reactions) {
                this.reactions.add(this, basic_reaction.emoji?.id, basic_reaction);
            }
        }
        ...
        if (basic.referenced_message !== undefined) this.replied_to = basic.referenced_message ? this.client.messages.add(basic.referenced_message) : null;
    }

    async reply(content: string|RichEmbed|MessageOptions, files: AttachmentInfo[] = []) {
        if (typeof content === "string") {
            return await this.channel.send({ content, reply_to: this }, files);
        } else if (content instanceof RichEmbed) {
            return await this.channel.send({ embed: content, reply_to: this }, files);
        } else {
            return await this.channel.send({ ...content, reply_to: this }, files);
        }
    }
