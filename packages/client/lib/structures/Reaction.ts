...
export class Reaction extends Identifiable<IdentifiableReaction> {
    ...
    constructor(
        ...
        basic: Partial<IdentifiableReaction>
    ) {
        super(client, basic);
    }
    ...
    patch(basic: Partial<IdentifiableReaction>) {
        ...
        if (basic.emoji !== undefined)
            this.emoji = this.client.emojis.add(basic.emoji);
    }

    async getUsers(pagination?: IdentifiablePaginationOptions<ResolvableUser>) {
        const before = pagination?.before
            ? this.client.users.resolveID(pagination.before)
            : undefined;
        const after = pagination?.after
            ? this.client.users.resolveID(pagination.after)
            : undefined;
        ...
        const res = await this.client.make<BasicUser[]>(
            "GET",
            { query },
            ApiEndpoints.GetReactions,
            this.message.channel,
            this.message,
            this.emoji
        );

        const users = new MaskedCache<User>(
            this.client,
            this.client.users,
            new Set()
        );
