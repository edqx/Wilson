...
export class User extends Identifiable<BasicUser> {
    ...
    avatarUrl(format: ImageFormat = ImageFormat.PNG, dynamic = true, use_default = true) {
        if (!this.avatar_hash) {
            if (use_default) {
                const discrim_type = (this.discriminator as unknown as number) % 5;

                return BaseUrls.CDN + formatEndpoint(CdnEndpoints.DefaultUserAvatar, discrim_type, format);
            }
        } else if (this.avatar_hash.startsWith("a_") && dynamic) {
            return this.avatarUrl(ImageFormat.GIF, use_default);
        } else {
            return BaseUrls.CDN + formatEndpoint(CdnEndpoints.UserAvatar, this, this.avatar_hash, format);
        }
    }

    patch(basic: Partial<BasicUser>) {
        if (basic.username !== undefined) this.username = basic.username;
        if (basic.discriminator !== undefined) this.discriminator = basic.discriminator;
        ...
        if (basic.mfa_enabled !== undefined) this.mfa_enabled = basic.mfa_enabled;
